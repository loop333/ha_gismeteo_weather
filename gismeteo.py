...
from homeassistant.components.weather import (
    ...
    ATTR_FORECAST_TIME, ATTR_FORECAST_WIND_SPEED, PLATFORM_SCHEMA, WeatherEntity)
    ...
    def update(self):
        ...
        if self._mode == 'daily':
            for d in xml.findall('location/day[@descr]'):
                data_out = {}
                data_out[ATTR_FORECAST_TIME] = d.get('date')
        ...
        if self._mode == 'hourly':
            for f in xml.findall('location/day/forecast'):
                v = f.find('values')
                data_out = {}
                data_out[ATTR_FORECAST_TIME] = f.get('valid')
